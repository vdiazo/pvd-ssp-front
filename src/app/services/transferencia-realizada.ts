...
export class TransferenciaRealizadaService {
  ...
  insertarTransferenciaMef(pformData: any) {
    let headers = new Headers()
    let accessToken = this.authService.getAccessToken();
    headers.set("Authorization", `Bearer ${accessToken}`);
    let options = new RequestOptions({ headers: headers });
    let apiUrl1 = Settings.API_ENDPOINT + "api/insertarTransferenciaMef";
    return this.httpFile.post(apiUrl1, pformData, options);
  }

  anularTransferenciaMef(data: any) {
    //{"id_transferencia_mef":327,"usuario_eliminacion":"cortiz"}"}
    let entidad = {
      data: JSON.stringify(data)
    }
    return this.http.post(Settings.API_ENDPOINT + "api/anularTransferenciaMef", entidad);
  }

  listarTransferenciaMefPrincipal(pTransferenciaRealizadaRequest: TransferenciaRealizadaRequest) {
    return this.http.get(Settings.API_ENDPOINT + 'api/listarTransferenciaMefPrincipal?strFechaPublicacionDesde=' + pTransferenciaRealizadaRequest.fecha_publicacion_desde + '&strFechaPublicacionHasta=' + pTransferenciaRealizadaRequest.fecha_publicacion_hasta + '&strDispositivo=' + pTransferenciaRealizadaRequest.dispositivo + "&intSkip=" + pTransferenciaRealizadaRequest.num_filas + "&intTake=" + pTransferenciaRealizadaRequest.num_pagina);
  }

  listarConvenioFaseProyectoMef(strCodUnificado:string,strNombreProyecto:string ) {
    return this.http.get(Settings.API_ENDPOINT + 'api/listarConvenioFaseProyectoMef?strCodUnificado=' + strCodUnificado+"&strNombreProyecto=" + strNombreProyecto);
  }
