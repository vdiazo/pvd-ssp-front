...
import { formatoMonedaPipe, formatoFechaPipe } from '../../appSettings';

@Component({
  selector: 'app-reporte-alta-direccion',
  templateUrl: './reporte-alta-direccion.component.html',
  styleUrls: ['./reporte-alta-direccion.component.css'],
  //encapsulation: ViewEncapsulation.None
})
export class ReporteAltaDireccionComponent implements OnInit {
  ...
  constructor(public funciones: Functions, private fs: FacadeService) { }
  ...
  ngOnInit() {
    ...
    //this.buscarProyecto(this.model);
  }
  ...
  exportarFormatoExcel(model) {
    ...
    this.fs.maestraService.listarReporteAltaDireccion(model.cod_snip, model.codigo_estado, model.id_municipalidad, model.coddepa, model.intSkip, model.intTake).subscribe(
      (respuesta: any) => {
        if (respuesta.reporte != null) {
          ...
          let worksheet = workbook.addWorksheet('Reporte_alta_direccion');

          // this.head1 = ["ESTRUCTURA DE INTERVENCIONES", "PLIEGO HABILITADO", "CODIGO DE RUTA", "SNIP", "CÓDIGO UNIFICADO",
          //   "ESTADO", "% AVANCE FÍSICO", "% AVANCE FINANCIERO", "META GLOBAL", "", "", "", "", "EJECUTADO", "", "PROGRAMADO",
          //   "", "MONTO TRANSFERENCIA", "DU/DS", "OBSERVACIÓN"];

          this.head1 = ["NOMBRE DEL PROYECTO", "PLIEGO HABILITADO", "CODIGO DE RUTA", "SNIP", "CÓDIGO UNIFICADO", "ESTADO", "% AVANCE FISICO", "% AVANCE FINANCIERO", "CONTRATO DE OBRA",
            "", "", "META GLOBAL", "", "", "", "", "EJECUTADO AÑO ANTERIOR", "",
            "EJECUTADO ACUMULADO ACTUAL", "", "", "PROGRAMADO", "", "MONTO DE TRANSFERENCIA", "DU/DS", "OBSERVACIONES"];
          ...
          this.head_merge1 =
            [
              ...
              { "columna": 9, "colspan": 2, "rowspan": 0 },
              { "columna": 12, "colspan": 4, "rowspan": 0 },
              { "columna": 17, "colspan": 1, "rowspan": 0 },
              { "columna": 19, "colspan": 2, "rowspan": 0 },
              { "columna": 22, "colspan": 1, "rowspan": 0 },
              { "columna": 24, "colspan": 0, "rowspan": 1 },
              { "columna": 25, "colspan": 0, "rowspan": 1 },
              { "columna": 26, "colspan": 0, "rowspan": 1 }];
          ...
          //   { "columna": 7, "colspan": 0, "rowspan": 1 },
          //   { "columna": 8, "colspan": 0, "rowspan": 1 },
          //   { "columna": 9, "colspan": 4, "rowspan": 0 },
          //   { "columna": 14, "colspan": 1, "rowspan": 0 },
          //   { "columna": 16, "colspan": 1, "rowspan": 0 },
          //   { "columna": 18, "colspan": 0, "rowspan": 1 },
          //   { "columna": 19, "colspan": 0, "rowspan": 1 },
          //   { "columna": 20, "colspan": 0, "rowspan": 1 }];



          // this.head2 = ["", "", "", "", "", "", "", "", "INICIO EJECUCIÓN", "PIM EJECUCIÓN (PLAZO CONTRACTUAL)",
          //   "METAS FÍSICAS", "UNID", "COSTO TOTAL ACTUAL(S/.)", "%", "EJECUTADO (S/.)", "%", "PIM TOTAL", "", "", ""];

          this.head2 = ["", "", "", "", "", "", "", "",
            "N° CONTRATO", "FECHA DE SUSCRPCIÓN",
            "VALOR ADJUDICADO",
            "INICIO EJECUCIÓN",
            "FIN EJECUCIÓN",
            "METAS FÍSICAS",
          ...
          this.head_merge2 =
            [
              ...
              { "columna": 23, "colspan": 0, "rowspan": 0 }
            ];
          ...
          //   "" + new formatoMonedaPipe(this.funciones).transform(this.funciones.fixed(this.total_costo, 2)), "",
          //   "" + new formatoMonedaPipe(this.funciones).transform(this.funciones.fixed(this.total_ejecutado, 2)), "",
          //   "" + new formatoMonedaPipe(this.funciones).transform(this.funciones.fixed(this.total_pim, 2)), 
          //   "" + new formatoMonedaPipe(this.funciones).transform(this.funciones.fixed(this.total_monto_transferencia, 2)), "", ""];

          // this.bodyTotal_merge =
          //   [{ "columna": 1, "colspan": 9, "rowspan": 0 },
          //   { "columna": 11, "colspan": 0, "rowspan": 0 },
          //   { "columna": 12, "colspan": 0, "rowspan": 0 },
          //   { "columna": 13, "colspan": 0, "rowspan": 0 },
          //   { "columna": 14, "colspan": 0, "rowspan": 0 },
          ...
          //   { "columna": 17, "colspan": 0, "rowspan": 0 },
          //   { "columna": 18, "colspan": 0, "rowspan": 0 },
          //   { "columna": 19, "colspan": 1, "rowspan": 0 }];
          ...
          // const bodyFila = this.bodyTotal;
          // let bodyTotalFila = worksheet.addRow(bodyFila);

          // bodyTotalFila.eachCell((cell, number) => {
          //   this.asignarRowsColsPan(number, cell, worksheet, this.bodyTotal_merge);
          //   cell.font = { name: 'Segoe UI', color: { argb: 'FFFFFFFF' }, bold: true, size: 9 }
          //   cell.alignment = { horizontal: 'center', vertical: 'distributed', wrapText: true }
          //   cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } }
          // });
          ...
          workbook.xlsx.writeBuffer().then((data: any) => {
            ...
            fs.saveAs(blob, 'ReporteAltaDireccion_' + this.funciones.obtenerFechaDescargaGuardarArchivo() + '.xlsx');
          })
        }
      }
    );
  ...
  buscarProyecto(model) {
    // model.id_municipalidad == null ? model.id_municipalidad = 0 : model.id_municipalidad = model.id_municipalidad;
    // model.cod_snip == null || model.cod_snip == "" ? model.cod_snip = 0 : model.cod_snip = model.cod_snip;
    // model.coddepa == null ? model.coddepa = "" : model.coddepa = model.coddepa;
    // model.codigo_estado == null ? model.codigo_estado = "" : model.codigo_estado = model.codigo_estado;

    // this.fs.maestraService.listarReporteAltaDireccion(model.cod_snip, model.codigo_estado, model.id_municipalidad,
  ...
  buscarDataSource(model: Proyecto) {
    model.id_municipalidad == null ? model.id_municipalidad = 0 : model.id_municipalidad = model.id_municipalidad;
    model.cod_snip == null || model.cod_snip == 0 ? model.cod_snip = 0 : model.cod_snip = model.cod_snip;
    model.coddepa == null ? model.coddepa = "" : model.coddepa = model.coddepa;
    model.codigo_estado == null ? model.codigo_estado = "" : model.codigo_estado = model.codigo_estado;
    model.intTake = this.paginaActiva;
