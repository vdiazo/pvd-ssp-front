...
export class InfoPrincipalComponent implements OnInit {
  ...
  constructor(private fs: FacadeService,
    ...
    private svcMaestra: MaestraService,
  ...
  // limpiarControlesSessionStorageBusqueda(){
  ...
  IrEjecucionEstado(pIdSeguimientoMonitoreo, pIdTramo, pIdFase, idProyecto?: number, pidTipoFase?: number, pSnip?: number, pidMunicipalidad?: number, pNombreTipoFase?: string, es_suspension?: boolean, fecha_inicio_obra?: string) {
    ...
    let vIdTipoFase: number = (pidTipoFase != null ? pidTipoFase : 0);
    ...
    let vEsSuspension: boolean = es_suspension;
    if (pNombreTipoFase == "OBRA") {
      ...
      this.svcMaestra.consultDatosAuditoria(vIdFase).subscribe(
        respuesta => {
          ...
          sessionStorage.setItem("esSuspension", JSON.stringify(vEsSuspension));
          sessionStorage.setItem("fecha_inicio_obra", JSON.stringify(fecha_inicio_obra));
          this.router.navigate(["/monitoreo/proyecto/infogeneral", vIdSeguimientoMonitoreo, vIdTramo, vIdFase, idProyecto, vSnip, vIdMunicipalidad, vIdTipoFase]);
        }
      );
    }
      ...
      this.svcMaestra.consultDatosAuditoria(vIdFase).subscribe(
        respuesta => {
          sessionStorage.setItem("DatosAuditoria", JSON.stringify(respuesta));
          this.router.navigate(["/monitoreo/proyecto/sspset", vIdSeguimientoMonitoreo, vIdTramo, vIdFase, idProyecto, vSnip, vIdMunicipalidad, vIdTipoFase]);
        }
      );
    }
  }
  ...
  asignarParametrosBusquedaBusquedaInicial() {
    ...
    //let numPagina: number = Number(sessionStorage.getItem("p_numero_pagina_activa"))
  ...
  abrirConsultaCU(cod) {
    window.open("http://ofi5.mef.gob.pe/invierte/ejecucion/verFichaEjecucion/" + cod, '_blank');
  }
  ...
  modalPDF(obj) {
    ...
    this.svcMaestra.AyudaMemoria(obj.id_tramo)
      .pipe(take(1))
      .subscribe(
        data => {
          entidadAyuda = data;
  ...
  obtenerCabeceraGrilla(pArrExportacion) {
    ...
    this.arrColsRowsPanPrimeraFila.push({ "columna": 1, "colspan": 0, "rowspan": 2 }
    ...
    this.arrCabeceraPrimeraFila = ["Naturaleza / Descripción del Producto"
      , "UEI Responsable"
    ...
    this.arrCabeceraPrimeraFila.push("Total (Soles)"
      , "% Avance de Ejecución de la Transferencia (Del Proyecto de Inversión)"
    ...
    this.arrColsRowsPanSegundaFila.push({ "columna": 4, "colspan": 0, "rowspan": 1 }
    ...
    //fin
    this.arrCabeceraSegundaFila.push("", "", "Tipo Servicio", "Numero de Procedimiento de Selección", "¿El Contrato es Exclusivo de la Inversión", "Número de Contrato", "RUC del Proveedor", "Nombre de Proveedor", "");
  ...
  exportarReportePrincipalGeneral(model) {
