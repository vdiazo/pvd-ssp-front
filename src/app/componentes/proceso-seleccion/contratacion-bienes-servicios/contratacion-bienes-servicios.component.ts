...
@Component({
  selector: 'app-contratacion-bienes-servicios',
  templateUrl: './contratacion-bienes-servicios.component.html',
  styleUrls: ['./contratacion-bienes-servicios.component.css']
})
export class ContratacionBienesServiciosComponent implements OnInit {
  snip: number;
  idTramo: number;
  idFase: number;
  idTipoFase: number;
  ...
  ngOnInit() {
    ...
    this.habilitar = this.autService.hasClaim("monitoreo_tabSel_bieyser_modComentario") == AplicarTipoControl.Visible

    // this.beProcesoSeleccionBienesServicios = new ProcesoSeleccionBienesServicios();
    // this.lstProcesoSeleccion.push({ "nomenclatura": "Prueba 1", "comentario": "", "estado": false,"id_comentario": 0, "accion": "G" },
    //   { "nomenclatura": "Prueba 2", "comentario": "","comentario_temporal": "", "estado": false,"id_comentario": 0, "accion": "G" },{ "nomenclatura": "Prueba 3", "comentario": "Prueba 3", "estado": false,"id_comentario": 1, "accion": "E" });
    this.busqueda(1);
    this.consultarNombreMuni();
  }

  consultarProcesoSeleccion(parametros: ProcesoSeleccionBienesServiciosRequest, pIdProyecto) {
    //parametros.id_municipalidad =  this.idMunicipalidad;
    this.servicio.listarProcesoSeleccion(parametros).subscribe(
      data => {
        ...
        //this.lstProcesoSeleccionCabecera = lstProcesoSeleccionCabeceraReturn.data;
        this.lstProcesoSeleccionCabecera = lstProcesoSeleccionCabeceraReturn.resultado;
        //this.totalRegistros = lstProcesoSeleccionCabeceraReturn.cantidad_registro;
  ...
  ConsultaProcesoSeleccionComentario(pIdProyecto, pIdMunicipalidad,pIdTipoFase) {
    this.servicio.listarProcesoSeleccionComentario(pIdProyecto, pIdMunicipalidad,pIdTipoFase).subscribe(
      data => {
  ...
  cambiarPagina(pagina) {
    ...
    this.entidadBusqueda.numero_Pagina = ((pagina.page * 5) - 5); //((pagina.page * 5) - 4);
  ...
  public busqueda(pagina) {
    ...
    this.entidadBusqueda.snip = this.snip == null ? 0 : this.snip;
    this.entidadBusqueda.tipo = TipoObjeto.BIENES_SERVICIOS;
    this.entidadBusqueda.numero_Pagina = ((pagina * 5) - 5); //((pagina * 5) - 4);
    this.entidadBusqueda.num_filas = 5;
    this.entidadBusqueda.codigo_unificado = this.codigo_unificado;
    this.entidadBusqueda.id_fase=this.idFase;
    this.consultarProcesoSeleccion(this.entidadBusqueda, this.idProyecto);
  ...
  openModalListCrono(identificador:any){
    let config={
      ignoreBackdropClick: true,
      keyboard: false,
      class: 'modal-lg',
      initialState: {
        codigoconvocatoria:identificador
      }
    }
    this.bsModalRef=this.modalService.show(ModalCronoOsceComponent,config);
  }
}