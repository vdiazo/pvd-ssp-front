...
//import { FacadeService } from '../../../../../core/patterns/facade.service';
//import { FiltroExpresionInteres } from '../../../../../core/models/actos-preparatorios/expresion-interes-filtro';
//import { Funciones } from '../../../../../shared/funciones/funciones';
//import { RegistrarExpresionInteres, PaqueteExpresionInteres } from '../../../../../core/models/actos-preparatorios/registrar-expresion-interes';
import { FacadeService } from 'src/app/patterns/facade.service';
import { FiltroExpresionInteres } from 'src/app/models/response/expresion-interes-filtro';
import { Functions } from 'src/app/appSettings';
import { RegistrarExpresionInteres, PaqueteExpresionInteres } from 'src/app/models/response/registrar-expresion-interes';
...
export class ModalAsociacionExpInteresComponent implements OnInit {
  ...
  constructor(private modalRef: BsModalRef, private fs: FacadeService, private fb: FormBuilder, public funciones: Functions) { }
  ...
  seleccionExpresionInteres(e: any, registro: any) {
    ...
    //let pIdentificador = registro.identificador;

    //let pNroExpresionInteres = registro.nro_exp_interes;

    // if (this.lstExpresionesSeleccionados != null) {
    //   if (estado.target.checked) {
    //     this.lstExpresionesSeleccionados.push({nro_exp_interes: pNroExpresionInteres, identificador: pIdentificador,fecha_contrato_ps: registro.fecha_contrato_ps,fecha_publicacion_ps: registro.fecha_publicacion_ps, usuario_creacion: this.nomUsuario});
    //   } else {
    //     if (this.lstExpresionesSeleccionados != null && this.lstExpresionesSeleccionados != undefined) {
    //       for (let i = 0; i < this.lstExpresionesSeleccionados.length; i++) {
    //         if (this.lstExpresionesSeleccionados[i].identificador == pIdentificador) {
    //           this.lstExpresionesSeleccionados.splice(i, 1);
    //         }
    //       }
    //     }
    //   }
  ...
  grabar() {
    if (this.lstExpresionesSeleccionados != null) {
      if (this.lstExpresionesSeleccionados.length > 0) {
        ...
        // this.fs.actosPreparatorios.registrarExpresionInteres(this.beRegistrarExpresionInteres).subscribe(
        //   data => {
  ...
  ListarContratacionesSinVincular(skip: number) {
    this.fs.procesoSeleccionService.listarFaseIdentificadorExonerar(this.intIdFase,0, this.itemsPerPage, skip,
  ...
  Insertar(item){
    let param=[];
      param.push({"id_fase":this.intIdFase, "identificador":item.identificador, "id_objeto":item.id_objeto, "usuario_creacion":sessionStorage.getItem("IdUsuario")});
