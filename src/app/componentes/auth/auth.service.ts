...
import { IUser, IUserInfo, IPerfil } from '../../Interfaces';
import { sessionStorageItems, Settings, AplicarTipoControl } from 'src/app/appSettings';
...
export class AuthService {
  ...
  */
  ...
  getMenu(IdPErfil: number): Observable<AppUserAuth> {
    return this.http.get<AppUserAuth>(Settings.API_ENDPOINT + "api/ListarMenuPerfil?intIdPerfil=" + IdPErfil).pipe(
      tap(respuesta => {
  ...
  getMenuLogon(IdUsuario: number, IdPErfil: number, token: string) {
