...
export class ModalCronogramaComponent implements OnInit {
  ...
  // construirCronogramaContratoPrincipal(infoPeriodos: any): void {
  //   let cronograma: FormArray = <FormArray>this.cronogramaForm.get("cronograma");
  ...
  //   if(infoPeriodos.paralizaciones != null){
  //     infoPeriodos.periodos.forEach(element => {
  //       let itemPeriodo = infoPeriodos.paralizaciones.find(x => x.paralizacion == element.periodo);
  //       if(itemPeriodo == undefined){
  //         cronograma.push(this.fb.group({
  //           periodo: element.periodo,
  //           fechaPeriodo: this.funciones.obtenerPeriodo(element.periodo),
  //           valoracion_programada: 0,
  //           avance_programada: 0,
  //           usuario_creacion: 'admin'
  //         }))
  ...
  onChangeTipoDefinicion(tipoDefinicionValue: number | string) {
    if (tipoDefinicionValue == "-1" || tipoDefinicionValue == -1) {
      ...
      // if(tipoDefinicionValue == 0){
      //   this.construirCronogramaContratoPrincipal(item);
      // }else{
