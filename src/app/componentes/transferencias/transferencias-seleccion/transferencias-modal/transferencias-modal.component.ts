...
import { TransferenciaRealizada } from '../../../../models/response/transferencia-realizada';
import { FacadeService } from '../../../../patterns/facade.service';
import { TransferenciaMaestra } from '../../../../models/response/transferencia-maestra';
...
@Component({
  ...
  styleUrls: ['./transferencias-modal.component.css']
})
export class TransferenciasModalComponent implements OnInit {
  ...
  ngOnInit() {
    this.model = new TransferenciaRealizada();
    this.model.id_transferencia = this.modelTransferencia.id_transferencia;
    this.listarFuenteFinanciamiento();
    this.model._BE_Tm_Ssppvd_Transferencia = new TransferenciaMaestra();
  ...
  administrarConfirmarTransferencia(model) {
    //registrar
    this.model._BE_Tm_Ssppvd_Transferencia.fecha_publicacion = model.fecha_publicacion;
    this.model._BE_Tm_Ssppvd_Transferencia.dispositivo = model.dispositivo;
    this.model._BE_Tm_Ssppvd_Transferencia.usuario_creacion = sessionStorage.getItem("Usuario");
      ...
      if(this.arrArchivosEnvio != null){
        if(this.arrArchivosEnvio.length > 0){
          ...
          for(let i=0; i < this.listSeleccionProyectos.length; i++){
            model.ListBE_Td_Ssppvd_Transferencia_Convenio.push({"id_convenio": this.listSeleccionProyectos[i].id_convenio
          ...
          if (model.id_transferencia == 0) {
            this.model._BE_Tm_Ssppvd_Transferencia.monto = sumMonto;
            ...
            this.formData.append('_BE_Tm_Ssppvd_Transferencia', this.devolverParametroJson(this.model._BE_Tm_Ssppvd_Transferencia));
            this.formData.append('ListBE_Td_Ssppvd_Transferencia_Convenio', this.devolverParametroJson(this.model.ListBE_Td_Ssppvd_Transferencia_Convenio));
            ...
            this.fs.transferenciaRealizadaService.registrarTransferenciaRealizadaHttpPostFile(this.formData).subscribe(
              respuesta => {
