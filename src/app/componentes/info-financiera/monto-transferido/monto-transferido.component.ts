...
@Component({
  selector: 'app-monto-transferido',
...
export class MontoTransferidoComponent implements OnInit {
  ...
  listarTransferencia(idTramo, idMunicipalidad, numFilas, numPagina) {
    this.servicio.listarTransferenciaXidFaseXidMunicipalidad(idTramo, idMunicipalidad, numFilas, numPagina).subscribe(
      respuesta => {
  ...
  openModalDetalleTransferencia(model) {
    this.config = {
      ...
      keyboard: false,
      initialState: {
        model: model, idTramo: this.idFase, idMunicipalidad: this.idMunicipalidad
      }
    };
