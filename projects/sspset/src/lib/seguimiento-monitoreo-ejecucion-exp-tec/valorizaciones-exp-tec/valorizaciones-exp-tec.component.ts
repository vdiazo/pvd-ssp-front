...
import { ModalRegistrarComponent } from '../valorizaciones-exp-tec/modal-registrar/modal-registrar.component';
import { GaleriaComponent } from '../valorizaciones-exp-tec/galeria/galeria.component';
...
@Component({
  selector: 'set-valorizaciones-exp-tec',
  templateUrl: './valorizaciones-exp-tec.component.html',
  styleUrls: ['./valorizaciones-exp-tec.component.css']
})
export class ValorizacionesExpTecComponent implements OnInit {
  ...
  constructor(private modalService: BsModalService, private fs: FacadeService, private sMant: MaestraService, public funciones: Funciones) { }
  ...
  openModalAccSeguimientoObra() {
    ...
    this.entidad.id_valorizacion_ejecucion_expediente = 0;
    this.entidad.id_seguimiento_ejecucion_expediente = this.idSeguimientoMonitoreoObra;
    this.config = {
      ignoreBackdropClick: true,
      keyboard: false,
      initialState: {
        //seguimientoMonitoreoAccion: this.entidad, fecha_inicio_contractual: this.model.fecha_inicio_contractual
        seguimientoMonitoreoAccion: this.entidad, fecha_inicio_contractual: this.fecha_inicio_contractual
      },
  ...
  listarAccionSeguimientoMonitoreoObra(id: number, skip: number, take: number) {
    ...
    // this.fs.valorizacionesService.listarValorizaciones(id, skip, take).subscribe(
    ...
    //     respuesta.valorizacion_ejecucion_expediente.map(
    //       (item) => {
  ...
  private setearMontosValorizaciones(respuesta: IAccionSeguimientoMonitoreo) {
    ...
    if (respuesta.cantidad_registro == 0) {
      ...
      this.listAccionSeguimientoMonitoreo = respuesta.valorizacion_ejecucion_expediente;
      ...
      if (respuesta.total_valorizacion_ejecucion_expediente != null) {
        this.montoTotalFisicoReal = respuesta.total_valorizacion_ejecucion_expediente.avance_fisico_real;
        this.montoTotalFisicoProgramado = respuesta.total_valorizacion_ejecucion_expediente.avance_fisico_programado;
        this.montoTotalFinancieroReal = respuesta.total_valorizacion_ejecucion_expediente.avance_financiero_real;
        this.montoTotalFinancieroProgramado = respuesta.total_valorizacion_ejecucion_expediente.avance_financiero_programado;
        this.montoTotal = respuesta.total_valorizacion_ejecucion_expediente.monto;
      }
    }
  }
  ...
  eliminarAccionSeguimientoMonitoreo(model) {
    ...
    let strData = { id_valorizacion_ejecucion_expediente: model.id_valorizacion_ejecucion_expediente, usuario_eliminacion: model.usuario_eliminacion }
    this.funciones.alertaRetorno("question", "¿Está seguro de eliminar el Avance de Obra?", "", true, (respuesta) => {
      if (respuesta.value) {
        this.fs.valorizacionesService.eliminarValorizacion(strData).subscribe(
          () => {
            this.listarAccionSeguimientoMonitoreoObra(model.id_seguimiento_ejecucion_expediente, this.num_filas, this.numero_Pagina);
            //this.consultaAuditoria("Valorizaciones");
          }
        );
      }
