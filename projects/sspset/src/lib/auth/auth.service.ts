...
export class AuthService {
  ...
  */
  ...
  getComponentes(IdPerfil: number): Observable<AppUserClaim[]> {
    return this.http.get<AppUserClaim[]>(Settings.API_ENDPOINT_SET + "api/listarComponentePerfil?intIdPerfil=" + IdPerfil);
  }

  getMenu(IdPErfil: number): Observable<AppUserAuth> {
    return this.http.get<AppUserAuth>(Settings.API_ENDPOINT_SET + "api/ListarMenuPerfil?intIdPerfil=" + IdPErfil).pipe(
      tap(respuesta => {
  ...
  getMenuLogon(IdUsuario: number, IdPErfil: number, token: string):any {
    ...
    return this.httpusuario.get(Settings.API_ENDPOINT_SET + "api/ListarMenuPerfil?intIdPerfil=" + IdPErfil + "&intIdUsuario=" + IdUsuario, options).pipe(
      tap(respuesta => {
        Object.assign(this.securityObject, respuesta);
      })
    );
