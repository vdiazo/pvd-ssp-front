...
@Component({
  selector: 'ssi-modal-pdf',
  templateUrl: './modal-pdf.component.html',
  styleUrls: ['./modal-pdf.component.css']
})
export class ModalPdfComponent implements OnInit {
  constructor(public modalRef: BsModalRef, private svcMAestra: MaestraSsiService, public funciones: Functions, private svcTramo: TramoGeograficoService, private fs: FacadeService, private LinkNubeService:LinkNubeService) { }
  idTramo: number;
  entidadAyuda: any = {};
  fecha: Date = new Date();
  actual: any;
  ...
  cargarGrafico() {
    this.fs.valorizacionService.listarValorizacionesGraficoSnip(parseInt(this.entidadAyuda.codigo_snip)).subscribe(
      respuesta => {
  ...
  ListarAyuda() {
    this.svcMAestra.AyudaMemoria(this.idTramo).subscribe(
      data => {
