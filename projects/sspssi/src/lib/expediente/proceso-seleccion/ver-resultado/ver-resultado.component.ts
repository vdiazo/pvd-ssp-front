...
import { ProcesoSeleccionResultado } from 'projects/sspssi/src/models/response/proceso-seleccion-resultado';
import { ModalGarantiasExpComponent } from './modal-garantias-exp/modal-garantias-exp.component';

defineLocale('es', deLocale);

@Component({
  selector: 'ssi-ver-resultado',
...
export class VerResultadoComponent implements OnInit {
  ...
  paginaActiva = 0;
  paginaActivaGarantia = 0;
  numero_Pagina = 0;
  numero_Pagina_Garantia = 0;
  num_filas = 5;
  verConvocado = false;
  verAdjudicado = false;
  verConsentido = false;
  verContratado = false;
  verLabelNullsFecha = true;
  tempNomMuni = '';
  nomemclaturaProc = '';
  urlGarantia = '';
  config;
  ...
  ngOnInit() {
    ...
    this.procesoSeleccionRequest.codigo_unificado = parseInt(sessionStorage.getItem('CodUnificado'), 10);
  ...
  editarGarantia(model) {
    ...
    this.bsModalRef = this.modalService.show(ModalGarantiasExpComponent, this.config);
