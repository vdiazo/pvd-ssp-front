...
import { ProcesoSeleccionBienesServiciosRequest } from 'projects/sspset/src/models/request/proceso-seleccion-bs-request';
...
import { VerResultadoComponent } from '../ver-resultado/ver-resultado.component';

@Component({
  selector: 'ssi-contratacion-consultoria',
...
export class ContratacionConsultoriaComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.habilitar = this.autService.hasClaim('monitoreo_tabSel_modComentario_exp_tec_eje_dir') == AplicarTipoControl.Visible;
    this.busqueda(1);
  ...
  openModalVerResultados(model) {
    ...
    this.bsModalRef = this.modalService.show(VerResultadoComponent, this.config);
  }
}
